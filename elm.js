...
Elm.Main.make = function (_elm) {
   ...
   var _op = {};
   var pickSize = F2(function (_p0,sizes) {
      var _p1 = _p0;
      var _p4 = _p1._0;
      var _p3 = _p1._1;
      var sizeRating = function (size) {
         var penalty = _U.cmp(size.width,_p4) > 0 || _U.cmp(size.height,_p3) > 0 ? 400 : 0;
         return $Basics.abs(_p4 - size.width) + $Basics.abs(_p3 - size.height) + penalty;
      };
      var _p2 = A2($List.sortBy,sizeRating,sizes);
      if (_p2.ctor === "::") {
            return $Task.succeed(_p2._0.source);
         } else {
   ...
   var selectPhoto = function (photos) {
      var _p5 = photos;
      if (_p5.ctor === "::") {
            return $Task.succeed(_p5._0);
         } else {
            return $Task.fail($Http.UnexpectedPayload("expecting 1 or more photos from Flickr"));
         }
   };
   var getFlickrUrlFor = F2(function (method,searchArgs) {
      return A2($Http.url,
      "https://api.flickr.com/services/rest/",
      A2($Basics._op["++"],
      _U.list([{ctor: "_Tuple2",_0: "format",_1: "json"}
              ,{ctor: "_Tuple2",_0: "nojsoncallback",_1: "1"}
              ,{ctor: "_Tuple2",_0: "api_key",_1: "9be5b08cd8168fa82d136aa55f1fdb3c"}
              ,{ctor: "_Tuple2",_0: "method",_1: A2($Basics._op["++"],"flickr.photos.",method)}]),
      searchArgs));
   });
   var createHttpTaskSignal = F3(function (getImage,sampled,events) {    return A3($Signal.map2,getImage,sampled,events);});
   var createSignalForImageRetrievalTask = F2(function (getImage,querySignal) {    return A3(createHttpTaskSignal,getImage,$Window.dimensions,querySignal);});
   var queryMailbox = $Signal.mailbox("Jupiter");
   var resultsMailbox = $Signal.mailbox("");
   var imgStyle = F2(function (height,src) {
      return _U.list([{ctor: "_Tuple2",_0: "background-image",_1: A2($Basics._op["++"],"url(\'",A2($Basics._op["++"],src,"\')"))}
                     ,{ctor: "_Tuple2",_0: "background-repeat",_1: "no-repeat"}
                     ,{ctor: "_Tuple2",_0: "background-attachment",_1: "fixed"}
                     ,{ctor: "_Tuple2",_0: "background-position",_1: "center"}
                     ,{ctor: "_Tuple2",_0: "width",_1: "100%"}
                     ,{ctor: "_Tuple2",_0: "height",_1: A2($Basics._op["++"],$Basics.toString(height),"px")}]);
   });
   var myStyle = _U.list([{ctor: "_Tuple2",_0: "width",_1: "100%"}
                         ,{ctor: "_Tuple2",_0: "height",_1: "40px"}
                         ,{ctor: "_Tuple2",_0: "padding",_1: "10px 0"}
                         ,{ctor: "_Tuple2",_0: "font-size",_1: "2em"}
                         ,{ctor: "_Tuple2",_0: "text-align",_1: "center"}]);
   var view = F3(function (height,keyword,imgUrl) {
      return A2($Html.div,
      _U.list([$Html$Attributes.style(A2(imgStyle,height,imgUrl))]),
      _U.list([A2($Html.input,
      _U.list([$Html$Attributes.placeholder("Github User Name")
              ,$Html$Attributes.value(keyword)
              ,A3($Html$Events.on,"input",$Html$Events.targetValue,$Signal.message(queryMailbox.address))
              ,$Html$Attributes.style(myStyle)]),
      _U.list([]))]));
   });
   var main = A4($Signal.map3,view,$Window.height,queryMailbox.signal,resultsMailbox.signal);
   var Size = F3(function (a,b,c) {    return {source: a,width: b,height: c};});
   var sizeDecoder = function () {
      ...
      A2($Json$Decode._op[":="],"width",number),
      A2($Json$Decode._op[":="],"height",number));
   }();
   var sizeListDecoder = A2($Json$Decode.at,_U.list(["sizes","size"]),$Json$Decode.list(sizeDecoder));
   var Photo = F2(function (a,b) {    return {id: a,title: b};});
   var photoDecoder = A3($Json$Decode.object2,Photo,A2($Json$Decode._op[":="],"id",$Json$Decode.string),A2($Json$Decode._op[":="],"title",$Json$Decode.string));
   var photoListDecoder = A2($Json$Decode.at,_U.list(["photos","photo"]),$Json$Decode.list(photoDecoder));
   var getImage = F2(function (dimensions,keyword) {
      var searchArgs = _U.list([{ctor: "_Tuple2",_0: "sort",_1: "random"},{ctor: "_Tuple2",_0: "per_page",_1: "10"},{ctor: "_Tuple2",_0: "tags",_1: keyword}]);
      return A2($Task.andThen,
      A2($Task.andThen,A2($Http.get,photoListDecoder,A2(getFlickrUrlFor,"search",searchArgs)),selectPhoto),
      function (photo) {
         return A2($Task.andThen,
         A2($Http.get,sizeListDecoder,A2(getFlickrUrlFor,"getSizes",_U.list([{ctor: "_Tuple2",_0: "photo_id",_1: photo.id}]))),
         pickSize(dimensions));
      });
   });
   var requestImgs = Elm.Native.Task.make(_elm).performSignal("requestImgs",
   function () {
      var imageRetrievalSignal = A2(createSignalForImageRetrievalTask,getImage,queryMailbox.signal);
      return A2($Signal.map,
      function (httpTask) {
         return A2($Task.andThen,httpTask,$Signal.send(resultsMailbox.address));
      },
      A2($Signal.sampleOn,queryMailbox.signal,imageRetrievalSignal));
   }());
   return _elm.Main.values = {_op: _op
                             ...
                             ,imgStyle: imgStyle
                             ,main: main
                             ,resultsMailbox: resultsMailbox
                             ,queryMailbox: queryMailbox
                             ,createSignalForImageRetrievalTask: createSignalForImageRetrievalTask
                             ,createHttpTaskSignal: createHttpTaskSignal
                             ,getImage: getImage
                             ,photoListDecoder: photoListDecoder
                             ,photoDecoder: photoDecoder
                             ,sizeListDecoder: sizeListDecoder
                             ,sizeDecoder: sizeDecoder
                             ,getFlickrUrlFor: getFlickrUrlFor
